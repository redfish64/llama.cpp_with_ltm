...
struct ggml_cgraph * build_graph(const simple_model& model) {
    ...
    struct ggml_tensor * a_t = ggml_transpose(ctx0, model.a);
    a_t = ggml_dup(ctx0,a_t);

    struct ggml_tensor * result2 = ggml_mul_mat(ctx0,a_t, result);
...
struct my_result compute(const simple_model & model, ggml_gallocr_t allocr) {
    ...
    return {ggml_graph_node(gf, -2),ggml_graph_node(gf, -1)};
}

void print_result(char *name, ggml_tensor *result)
{
    ...
    printf("%s: mul mat (%d x %d) (transposed result):\n[", name, (int) result->ne[0], (int) result->ne[1]);
...
int main(void) {
    ...
    // perform computation
    my_result my_r = compute(model, allocr);

    print_result("r1",my_r.r1);
    print_result("r2",my_r.r2);
